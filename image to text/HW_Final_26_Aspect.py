...
params = {'num_classes' : len(parameters) + 1,
          'path' : r'E:\CRNN_code\IAMhand2printv5',
          'model_path': './model_weights26_Aspect_Final',
          'Batch_Size': 4,
          'lr': 0.0001,
          'num_epochs':1000,
          'width_thresh':250
...
def get_file_paths(path=params['path']):
    ...
    WordList = np.array([os.path.basename(i).split('_')[-1][:-4].lower() for i in paths])
...
def load_img(path):
    ...
    r_ = 32
    c_ = round(c/r * r_)
    if c_ > params['width_thresh']:
        c_ = params['width_thresh']
    img_ = cv2.resize(img_, (c_, r_))
    img_ = cv2.copyMakeBorder(img_, 0, 0, 0, 250 - c_, cv2.BORDER_CONSTANT, value=(255,255,255))
    return img_/255.0, c_

def Load_Batch(batch_paths):
    batch_data = []
    Image_Width = []
    for path in batch_paths:
        img_, seq_ = load_img(path)
        batch_data.append(img_)
        Image_Width.append(seq_)
    return np.array(batch_data), np.array(Image_Width)
...
def word2Sparse(labels):
    ...
    for n, word in enumerate(labels):
        for i, char in enumerate(list(word)):
            indices.append([n, i])
            if (letter2num[char] > 25) or (letter2num[char] < 0):
                print('Error')
                print(word)
...
def Architecture(Input, Seq_Len, is_training = True):
    ...
    with tf.variable_scope("Network"):
        with tf.variable_scope("CNN"):
            ...
            b_norm8 = tf.nn.relu(tf.layers.batch_normalization(conv8, training=is_training, name='batch-norm3'),
                                 name='relu8')
            conv_reshaped = tf.squeeze(b_norm8, axis = [1], name='reshaped')
            print(conv_reshaped)

            #shape = b_norm8.get_shape().as_list()
            #transposed = tf.transpose(b_norm8, perm=[0, 2, 1, 3], name='transposed')
            #conv_reshaped = tf.reshape(transposed, [-1, time_step, shape[1] * shape[3]], name='reshaped')
...
def Compile():
    ...
    logits = Architecture(Input, Seq_Len)
    #print(logits)
    ...
    for i in range(params['num_epochs']):  # range(1): #range(params['num_epochs']):
        ...
        for idx in range(Total_Data // params['Batch_Size']):  # range(1): #
            ...
            batch_data, Image_Width = Load_Batch(batch_paths)
            batch_SeqL = np.floor(Image_Width / 2) - 1

            feed_dict = {Input: batch_data, Target: batch_target, Seq_Len: batch_SeqL}
            ...
            if itr % 1000 == 0:
                ...
                if prev_accu < np.mean(tes_accu):
                    save(saver, params['model_path'], itr)
                    print('Model Saved!!')
                    prev_accu = np.mean(tes_accu)
                    


def get_graph(model_dir = params['model_path']):
...
def Test(graph):
    ...
    for idx in range(Total_Data // params['Batch_Size']):  #
        ...
        feed_dict = {Input: batch_data, Seq_Len: batch_SeqL}
        ...
        res = np.array([i == j for i, j in zip(batch_Words, Predicted_Words)]).astype(np.int8)
        tes_accu.append(res)
        print('### Testing Results:  Batch:' + str(idx) + ' Accuracy: ' + str(np.mean(res)))
        # print('Done!')
        # print(pandas.DataFrame([batch_Words[:1], Predicted_Words[:1]], ['Label', 'Predicted']))

    print('### Final Mean Testing Accuracy:' + str(np.mean(tes_accu)))
